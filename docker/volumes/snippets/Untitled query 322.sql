...
--
-- Two helper functions used by every trigger function:
--   get_edge_function_url(fn_name) — builds the full edge function URL
--   get_service_role_key()         — returns the service role key
--
-- Both are SECURITY DEFINER and REVOKED from authenticated/anon
-- so only trigger functions (running as the DB superuser) can call them.
--
-- ⚠️  UPDATE get_service_role_key() with your actual service role key.
--     Get it from your .env file (SERVICE_ROLE_KEY).
-- ================================================================
...
CREATE OR REPLACE FUNCTION public.get_edge_function_url(fn_name TEXT)
  ...
  SET search_path = ''
AS $$
  SELECT 'https://api.skillima.com/functions/v1/' || fn_name;
$$;

REVOKE EXECUTE ON FUNCTION public.get_edge_function_url(TEXT)
  FROM authenticated, anon;

-- Returns the service role key used to authenticate edge function calls.
-- Hardcoded because ALTER SYSTEM SET app.* requires custom GUC parameters
-- declared in postgresql.conf — not available by default on self-hosted Supabase.
-- Function body is only readable by superusers, not by authenticated/anon.
CREATE OR REPLACE FUNCTION public.get_service_role_key()
  ...
  SET search_path = ''
AS $$
  SELECT 'your-service-role-key-here';  -- ⚠️ replace with your SERVICE_ROLE_KEY
$$;
...
-- SECURITY INVOKER (the default, but stated explicitly to satisfy the linter):
-- The view runs as the QUERYING USER, not the view creator.
-- RLS policies on project_enrollments, stage_progress, student_profiles,
-- and mentor_profiles all apply normally. Admins see everything;
-- regular users only see rows their RLS policies permit.
CREATE OR REPLACE VIEW public.github_integration_status
...
COMMENT ON VIEW public.github_integration_status IS
  'Admin monitoring view (SECURITY INVOKER). '
  'Respects RLS on all underlying tables — admins see all rows, '
  'other roles see only what their policies permit. '
  'Look for: github_status = error, stages_missing_pr > 0, '
